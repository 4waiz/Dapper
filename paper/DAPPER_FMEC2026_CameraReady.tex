...
\usepackage{flushend}
% hyperref is loaded last so it can patch the packages above. Set
% urlcolor=black if the proceedings require unlinked/black text in print.
...
\author{%
\IEEEauthorblockN{Awaiz Ahmed}
\IEEEauthorblockA{Al Ain University\\
Abu Dhabi, United Arab Emirates\\
Email: \href{mailto:awaiz42ad@gmail.com}{awaiz42ad@gmail.com}}
\and
\IEEEauthorblockN{Khubaib Amjad Alam}
\IEEEauthorblockA{Al Ain University\\
Abu Dhabi, United Arab Emirates\\
Email: \href{mailto:khubaib.alam@aau.ac.ae}{khubaib.alam@aau.ac.ae}}
}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.93\textwidth]{figures/dapper_architecture.pdf}
...
\begin{table*}[t]
\centering
\caption{Repeated evaluation over 30 independent seeds (1,000 frames per seed/profile/policy, $D=100$~ms). CIs are 95\% percentile bootstrap intervals over seeds. Confidence is the synthetic proxy held by the control loop at the deadline, not detector accuracy.}
\label{tab:main}
\scriptsize
...
\begin{minipage}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=0.93\linewidth]{figures/fig2_p95_latency_panel.pdf}\\
    \small (a) p95 control-output latency.
\end{minipage}\hfill
\begin{minipage}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=0.93\linewidth]{figures/fig2b_usable_confidence_panel.pdf}\\
...
\begin{minipage}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=0.72\linewidth]{figures/fig3_deadline_mode_distribution.pdf}\\
    \small (a) DAPPER mode share versus control deadline.
\end{minipage}\hfill
\begin{minipage}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=0.72\linewidth]{figures/runtime_bias.pdf}\\
...
Over 200,000 timed calls after warm-up, one DAPPER decision takes 1.45~$\mu$s on average (median 1.40, p95 1.80, p99 2.00~$\mu$s) in CPython on the evaluation workstation, and running DAPPER instead of local-only adds 1.31~$\mu$s per frame to benchmark wall time. These measure software overhead on a desktop processor, not robot hardware or application inference latency. Depending on profile, DAPPER changes mode 12--418 times per 1,000 frames; no physical switching cost is modeled.
